...
Function Target_TestIntegration {
    $targetDotnetTestIntegration = "DotNet.TestIntegration"
    if (IsTargetCompleted $targetDotnetTestIntegration) { return }
    DependsOn Target_Build

    LogTarget $targetDotnetTestIntegration
    $projects = Get-ChildItem -Path $srcDir -Filter "*Test.Integration.csproj" -Recurse -File
    Foreach ($projectFile in $projects) {
        Step_DotnetTest $projectFile
    }
    MarkTargetAsCompleted $targetDotnetTestIntegration
}
...
Function Target_FullBuild {
    ...
    DependsOn Target_TestIntegration
